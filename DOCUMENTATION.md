...
**التطهير النهائي للخدمات**: تم استئصال طبقة الخدمات (Services) من واجهة المستخدم تماماً. كافة المكونات تتصل الآن مباشرة بـ API Wall عبر كائن `api` الموحد. تم توحيد منطق الـ CSV في مكتبة أدوات سيادية. النظام الآن يتبع مساراً واحداً وحتمياً للبيانات.
...
---
**تحذير معمارية:** أي محاولة لاستخدام Supabase client أو استيراد مستودع أو استدعاء "خدمة" قديمة داخل مكون عميل ستعتبر خرقاً أمنياً وسيتم إبادتها فوراً. الطبقة الوحيدة المسموح بالتعامل معها هي `/api` و `api-client`.
