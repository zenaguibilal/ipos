    ...
    return (
        <div ref={ref} className="p-4 w-full text-black bg-white font-sans">
            ...
            <div className="text-xs space-y-1 mb-4">
                <p>Référence Retour: <strong>#{productReturn.uuid.substring(0, 8).toUpperCase()}</strong></p>
                <p>Facture Originale: <strong>#{productReturn.originalInvoiceNumber}</strong></p>
                <p>Date: {format(safeToDate(productReturn.createdAt!), 'dd/MM/yyyy HH:mm', { locale: fr })}</p>
            </div>
            ...
            <div className="space-y-1 border-t-2 border-black pt-2 text-xs">
                ...
                <div className="flex justify-between text-blue-600">
                    <span>Remboursé en espèces:</span>
                    <span>- {formatCurrency(productReturn.amountRefunded)}</span>
                </div>
                {impactDebt > 0.01 && (
                    <div className="flex justify-between text-green-600 font-bold border-t border-dashed pt-1">
            ...
            {productReturn.notes && (
                <div className="mt-4 p-2 bg-gray-100 text-[10px] italic border rounded">
                    <strong>Motif du retour :</strong> {productReturn.notes}
                </div>
            )}

            <footer className="text-center mt-8 text-[10px] text-gray-500 border-t pt-2">
                <p>Document généré par iPOS - Merci de votre confiance.</p>
