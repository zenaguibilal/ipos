      ...
      <Table>
        <TableHeader className="bg-white/5">
          <TableRow className="hover:bg-transparent border-white/5">
            ...
            <TableHead className="font-black uppercase tracking-widest text-[10px] text-muted-foreground text-center">Horodatage</TableHead>
              ...
              >
                {onToggleSelection && (
                    <TableCell className="px-6" onClick={(e) => e.stopPropagation()}>
                        <Checkbox checked={isSelected} onCheckedChange={() => onToggleSelection(expense.uuid)} />
                    </TableCell>
                )}
                <TableCell className="px-8 py-5">
                    <div className="flex items-center gap-4">
                        ...
                        <span className="font-black text-sm tracking-tight text-foreground uppercase truncate max-w-[300px]">{expense.description}</span>
                    </div>
                </TableCell>
                <TableCell>
                    ...
                    {isManagerOrAdmin && (
                        <DropdownMenu>
                            ...
                            <DropdownMenuContent align="end" className="luxury-glass p-2 min-w-[160px] shadow-2xl border-white/10">
                                <DropdownMenuItem onClick={() => onEdit(expense)} className="rounded-xl py-2.5 font-bold gap-3">
                                    <Edit className="h-4 w-4 opacity-60" /> Modifier
                                </DropdownMenuItem>
                                <DropdownMenuItem onClick={() => onDelete(expense)} className="text-destructive focus:text-destructive focus:bg-destructive/10 rounded-xl py-2.5 font-bold gap-3">
