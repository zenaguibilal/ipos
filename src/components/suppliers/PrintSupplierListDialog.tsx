...
interface PrintSupplierListDialogProps {
    suppliers: Supplier[];
}

const PrintableSupplierList = React.forwardRef<HTMLDivElement, { suppliers: Supplier[], profile: CompanyProfile | null }>(({ suppliers, profile }, ref) => {
    ...
    return (
        <div ref={ref} className="p-8 bg-white text-black font-sans">
            <header className="text-center mb-8 border-b-2 border-black pb-4">
                ...
                <h2 className="text-lg font-bold text-gray-600 mt-1">Registre des Partenaires & Encours Fournisseurs</h2>
            ...
            <table className="w-full text-xs border-collapse">
                <thead>
                    <tr className="bg-gray-100 border-b-2 border-black">
                        <th className="border p-3 text-left">Nom du Fournisseur</th>
                        <th className="border p-3 text-left">Contact / Tél</th>
                        <th className="border p-3 text-right">Dernière MAJ</th>
                ...
                <tbody>
                    {suppliers.map(s => (
                        <tr key={s.uuid} className="border-b border-gray-200">
                            <td className="border p-3 font-bold uppercase">{s.name}</td>
                            <td className="border p-3">
                                <div className="flex flex-col">
                                    <span className="font-medium">{s.contactPerson || '-'}</span>
                                    <span className="text-[10px] opacity-70">{s.phone || 'Pas de tél.'}</span>
                                </div>
                            </td>
                            <td className="border p-3 text-right opacity-60">
                                {format(new Date(s.updatedAt), 'dd/MM/yyyy')}
                            </td>
                            <td className={`border p-3 text-right font-black ${s.balance > 0 ? 'text-red-600' : 'text-green-600'}`}>
                                {s.balance.toFixed(1)}
                            </td>
                        </tr>
                    ))}
                ...
                <tfoot>
                    <tr className="bg-black text-white font-black">
                        <td className="p-4 text-right" colSpan={3}>DETTE TOTALE FOURNISSEURS</td>
                        <td className="p-4 text-right text-lg">{totalDebt.toLocaleString('fr-FR')} DA</td>
                    </tr>
                </tfoot>
            </table>
            ...
            <footer className="mt-12 flex justify-between items-end border-t pt-4">
                <div className="text-[8px] text-gray-400 italic">
                    iPOS Cloud Authority - Terminal de Gestion Souverain
                </div>
...
PrintableSupplierList.displayName = 'PrintableSupplierList';

export function PrintSupplierListDialog({ suppliers }: PrintSupplierListDialogProps) {
                ...
                <DialogContent className="max-w-5xl h-[90vh] flex flex-col print-dialog-content">
                    <DialogHeader className="print-hide">
                        <DialogTitle className="flex items-center gap-2">
                            ...
                            Aperçu du Registre Fournisseurs
                        </DialogTitle>
                        <DialogDescription>Format A4 optimisé pour l'archivage comptable.</DialogDescription>
                    </DialogHeader>
                    <div className="flex-grow overflow-y-auto bg-muted/50 p-4 rounded-3xl border border-white/5 shadow-inner">
                        <div className="bg-white mx-auto shadow-2xl" style={{ width: '210mm', minHeight: '297mm' }}>
                            <PrintableSupplierList ref={printRef} suppliers={suppliers} profile={profile || null} />
                        </div>
                    </div>
