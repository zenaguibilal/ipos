...
import { Plus, Search, Users, FileDown, Filter, FileUp, SortAsc } from 'lucide-react';
...
const sortOptions: { [key: string]: string } = {
    ...
    'outstandingBalance_desc': 'Dette (décroissante)',
    'lastActivityDate_desc': 'Activité récente',
...
export default function CustomersPage() {
    ...
    const customers = useLiveQuery<CustomerWithSalesData[]>(
        () => dataService.getCustomers({ query: debouncedSearchQuery, status: filterStatus, sortBy }),
    ...
    const handleConfirmImport = async (confirmedData: { toAdd: any[], toUpdate: any[] }) => {
     ...
     const handleExport = async () => {
        toast.info("Préparation de l'exportation des clients...");
        try {
            const csvString = await dataService.exportCustomersToCSV();
            const blob = new Blob([`\uFEFF${csvString}`], { type: 'text/csv;charset=utf-8;' });
            const link = document.createElement("a");
            const url = URL.createObjectURL(blob);
            link.setAttribute("href", url);
            link.setAttribute("download", `export-clients-${new Date().toISOString().split('T')[0]}.csv`);
            ...
            toast.error("Erreur lors de l'exportation des clients.");
            console.error(error);
        }
    };
