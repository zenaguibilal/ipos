...
import { Loader2, Lock, Mail, Building2, UserPlus, ShieldCheck, ArrowRight } from 'lucide-react';
...
/**
 * @fileOverview AUTHENTICATION GATEWAY (LOGIN & SIGNUP)
 * التصميم الزجاجي الفاخر المتوافق مع معايير iPOS السحابية.
 */
...
export default function AuthPage() {
    ...
    const handleSignup = async (e: React.FormEvent) => {
        e.preventDefault();
        if (!companyName.trim()) {
            toast.error("Le nom de l'établissement هو مطلب سيادي.");
            return;
        }
        setIsLoading(true);
        try {
            await api.post('auth/signup', { email, password, companyName });
            toast.success("Votre compte souverain a été créé. Connectez-vous maintenant.");
            // Switch to login tab
