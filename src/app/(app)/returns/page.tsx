...
export default function ReturnsPage() {
    ...
    const handleExportToCSV = () => {
        ...
        const csvData = filteredReturns.map(r => {
            const returnDate = r.createdAt ? safeToDate(r.createdAt) : new Date();
            const itemsSummary = r.items.map(item => `${item.quantity} x ${item.productName}`).join('; ');

            return {
                "Date": format(returnDate, 'yyyy-MM-dd HH:mm:ss'), "Facture Originale": r.originalInvoiceNumber,
                "Client": r.customerName || 'N/A', "Valeur Retour": r.totalReturnValue,
                "Montant Remboursé": r.amountRefunded, "Articles": itemsSummary, "Notes": r.notes || '',
            };
        });
    ...
    return (
        <>
            {selectedReturn && (
                <ReturnDetailsDialog isOpen={!!selectedReturn} onOpenChange={(isOpen) => !isOpen && setSelectedReturn(null)} productReturn={selectedReturn} />
            )}
             {deletingReturn && (
                <DeleteReturnDialog isOpen={!!deletingReturn} onOpenChange={(isOpen) => !isOpen && setDeletingReturn(null)} productReturn={deletingReturn} />
            )}
                        ...
                        <DateRangePicker date={dateRange} setDate={setDateRange} />
                         <DropdownMenu>
                            ...
                            <DropdownMenuContent align="end">
                                <DropdownMenuItem onClick={handleExportToCSV}><Download className="mr-2 h-4 w-4" /> Exporter en CSV</DropdownMenuItem>
                            </DropdownMenuContent>
                        </DropdownMenu>
                        <Button asChild>
                            <Link href="/returns/new"><PlusCircle className="mr-2 h-4 w-4" />Enregistrer un retour</Link>
                        </Button>
                    </div>
                </div>

                <div className="grid gap-4 md:grid-cols-3 mb-6">
                    <Card><CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2"><CardTitle className="text-sm font-medium">Retours (filtrés)</CardTitle><Undo2 className="h-4 w-4 text-muted-foreground" /></CardHeader><CardContent><div className="text-2xl font-bold">{returnsCount}</div><p className="text-xs text-muted-foreground">Transactions de retour sur la période</p></CardContent></Card>
                    <Card><CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2"><CardTitle className="text-sm font-medium">Valeur retournée (filtrée)</CardTitle><CircleDollarSign className="h-4 w-4 text-muted-foreground" /></CardHeader><CardContent><div className="text-2xl font-bold text-destructive">{totalReturnedValue.toFixed(1)} DA</div><p className="text-xs text-muted-foreground">Valeur des produits retournés</p></CardContent></Card>
                    <Card><CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2"><CardTitle className="text-sm font-medium">Articles retournés (filtrés)</CardTitle><Hash className="h-4 w-4 text-muted-foreground" /></CardHeader><CardContent><div className="text-2xl font-bold">{totalItemsReturned}</div><p className="text-xs text-muted-foreground">Nombre d'articles retournés</p></CardContent></Card>
                </div>
                        ...
                        {isLoading ? (
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
                                {filteredReturns.map((r) => (
                                    <ReturnCard key={r.id} productReturn={r} onViewDetails={setSelectedReturn} onDelete={setDeletingReturn} />
                                ))}
                            </div>
                        )}
                    </CardContent>
