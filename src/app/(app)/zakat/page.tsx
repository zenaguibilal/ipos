...
export default function ZakatPage() {
    ...
    const handlePrintReport = (calcToPrint?: any) => {
    ...
    const liquidityIssue = useMemo(() => {
        if (!result || !result.isNisabReached) return false;
    ...
    return (
        <div className="p-4 sm:p-6 space-y-10 animate-in fade-in duration-1000 max-w-screen-2xl mx-auto pb-24 md:pb-10">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                {anniversaryInfo && (
                    <div className={cn(
                        "p-6 rounded-[2.5rem] border-2 flex items-center justify-between gap-6 transition-all duration-1000",
                        ...
                        <div className="flex items-center gap-6">
                            ...
                            <div>
                                ...
                                <p className="text-[9px] font-black uppercase tracking-[0.3em] opacity-60">Échéance : {anniversaryInfo.date.toLocaleDateString('fr-FR', { day: 'numeric', month: 'long' })}</p>
                            </div>
                        </div>
                        <div className="text-right">
                            <p className={cn("text-2xl font-black tracking-tighter", anniversaryInfo.daysLeft <= 7 ? "text-destructive" : "text-foreground")}>
                ...
                {liquidityIssue && (
                    <div className="p-6 rounded-[2.5rem] border-2 bg-orange-500/10 border-orange-500/30 flex items-center justify-between gap-6 animate-in slide-in-from-right-4 duration-1000 shadow-2xl shadow-orange-500/10">
                        <div className="flex items-center gap-6">
                            <div className="p-4 rounded-2xl bg-orange-500/20 text-orange-500 shadow-inner">
                                <AlertTriangle className="h-8 w-8 animate-bounce" />
                            </div>
                            <div>
                                <h3 className="text-xl font-black uppercase italic tracking-tighter text-orange-500">Alerte de <span className="text-foreground">Liquidité</span></h3>
                                <p className="text-[9px] font-black uppercase tracking-[0.3em] opacity-60">Le montant dû excède votre cash actuel</p>
                            </div>
                        </div>
                        <Badge className="bg-orange-500 text-white font-black uppercase text-[8px] h-6 px-3 rounded-lg">Action Requise</Badge>
                    </div>
                )}
            </div>
                ...
                <TabsContent value="calculator" className="space-y-10 outline-none animate-in slide-in-from-bottom-4 duration-700">
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                        ...
                        <Card className="luxury-glass border-white/5 bg-muted/10 overflow-hidden group shadow-2xl hover:border-primary/20 transition-all">
                            ...
                            <CardContent className="p-8 space-y-8">
                                ...
                                <div className="space-y-4">
                                    <Label htmlFor="cash" className="text-[10px] font-black uppercase tracking-widest opacity-70 ml-1">Cash en Caisse & Liquidités (DA)</Label>
                                ...
                                <CardContent className="p-10 space-y-10 relative z-10">
                                    <div className="grid grid-cols-2 gap-6">
                                        <div className="text-center p-4 rounded-2xl bg-background/40 border border-white/5 shadow-inner">
                                            <p className="text-[9px] font-bold text-muted-foreground uppercase tracking-widest mb-1.5">Nisab (85g Or)</p>
                                    ...
                                    {!result.isNisabReached && nisabGap > 0 && (
                                        <div className="p-5 rounded-2xl bg-destructive/5 border border-destructive/10 text-center animate-in zoom-in-95 duration-500">
                                            ...
                                            <p className="text-[8px] text-muted-foreground italic mt-1 uppercase">Manque à gagner pour atteindre le seuil</p>
                                        </div>
                                    )}
                                    ...
                                    <div className={cn(
                                        ...
                                        <div className="space-y-1">
                                            ...
                                            <p className="text-[10px] italic leading-tight font-medium">
                                                {result.isNisabReached ? "La فريضة est applicable sur votre patrimoine." : "Patrimoine inférieur au seuil de وجوب."}
                                            </p>
                                        </div>
                                    </div>
                                </CardContent>
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                        <Card className="luxury-glass border-white/5 bg-muted/10 overflow-hidden shadow-2xl">
                            <CardHeader className="bg-white/5 border-b border-white/5 py-6 px-8"><CardTitle className="text-sm font-black uppercase tracking-[0.3em] flex items-center gap-3"><PieChartIcon className="h-5 w-5 text-primary" />Analyse de Composition</CardTitle></CardHeader>
                        ...
                        <div className="p-8 rounded-[3rem] bg-emerald-500/5 border border-emerald-500/10 flex flex-col items-center justify-center text-center space-y-8 shadow-inner">
                            ...
                            <div className="space-y-4">
                                ...
                                <p className="text-[11px] text-muted-foreground leading-relaxed max-w-md uppercase font-bold opacity-70">
                                    Le calcul déterministe soustrait vos passifs (dettes fournisseurs و charges) de vos actifs circulants. 
                ...
                <TabsContent value="history" className="animate-in slide-in-from-bottom-4 duration-700 outline-none space-y-8">
                    <div className="flex flex-col sm:flex-row justify-between items-center gap-4 luxury-glass p-2 bg-muted/20 border-white/5 shadow-inner">
                        <div className="flex items-center gap-2 px-4 py-2 bg-background/40 rounded-xl border border-white/10">
                            ...
                            <span className="text-[10px] font-black uppercase tracking-widest">{zakatHistory.length} Points archivés</span>
                        </div>
                        <div className="flex items-center gap-3">
                            <Button variant="outline" onClick={handleExport} disabled={zakatHistory.length === 0} className="h-10 rounded-xl border-white/5 font-black uppercase text-[10px] tracking-widest gap-2 hover:bg-white/5">
                    ...
                    {zakatHistory.length === 0 ? (
                        <div className="py-40 text-center opacity-30 grayscale border-2 border-dashed rounded-[3rem] border-white/5 bg-white/5 space-y-6">
                            ...
                            <p className="text-2xl font-black uppercase tracking-widest">Registre d'Archives Vierge</p>
                        </div>
                                ...
                                <Card key={h.uuid} className="luxury-glass p-8 border-white/5 bg-muted/10 hover:border-emerald-500/30 transition-all group relative overflow-hidden flex flex-col justify-between h-64 cursor-pointer shadow-xl" onClick={() => handleViewHistory(h)}>
                                    ...
                                    <div className="space-y-2 relative z-10">
                                        <p className="text-[10px] font-black uppercase text-muted-foreground tracking-widest mb-4 flex items-center gap-2">
                                            ...
                                            {format(new Date(h.createdAt), 'dd MMMM yyyy', { locale: fr })}
                                        </p>
                                        <p className="text-3xl font-black text-emerald-500 tracking-tighter">{formatCurrency(h.zakatAmount)}</p>
                                        <p className="text-[10px] font-bold text-muted-foreground uppercase opacity-60 tracking-widest">Assiette: {formatCurrency(h.zakatBase)}</p>
                                    </div>
                        ...
                        <ZakatHistoryTable history={zakatHistory} onViewDetails={handleViewHistory} onPrint={handlePrintReport} />
                    )}
                </TabsContent>
            </Tabs>
